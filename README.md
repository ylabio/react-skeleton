...
`npm run build:server` - если нужен сервер рендера (SSR), тогда выполнится сборка браузерной и 
...
отдаст статичные файлы из ./dist/client/assets/, а также проксирует запросы к АПИ. 
