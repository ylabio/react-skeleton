...
function ModalsExamplePage(props: IProps) {
  ...
  return (
    <PageLayout>
      ...
      <p>
        Модальные окна отображаются поверх текущей страницы, делая элементы страницы недоступными пока
        окно не закроется.
        ...
        реализовано окно.
        Любой React компонент может стать окном, его достаточно прописать в импорте сервиса - в файле
        `@src/services/modals/imports.ts`.
        В компонент окна автоматически передаётся функция обратного вызова close, чтобы закрыть окно
        и передать результат.
        Смотрите README в `./src/services/modals/README.md`
      </p>
      <p>
